class Idris2Pack < Formula
  ...
  bottle do
    root_url "https://github.com/Portfoligno/homebrew-idris/releases/download/idris2-pack-2026.05.01"
    sha256 cellar: :any, arm64_sequoia: "5a8b8ce3426ab88839129778bf1e70bd2de7d8c16b4d09222eba8b8fa7e60c0a"
    sha256 cellar: :any, arm64_sonoma:  "d993538dfca5bb82f14e9d84c67a89d13e2e4e28c149f91e24bbe4b8dbeaa41c"
  end
  ...
  def install
    ...
    (bin/"pack").write "#!/bin/sh\n" \
      "LIBEXEC=#{libexec}\n" \
      "export PATH=#{chez_bin}:${LIBEXEC}/idris2-toolchain/bin:$PATH\n" \
      + <<~'WRAPPER'

        # Auto-align pack state with Homebrew-managed collection
        BREW_COLLECTION="$(cat "${LIBEXEC}/COLLECTION" 2>/dev/null)" || true
        BREW_IDRIS2_COMMIT="$(cat "${LIBEXEC}/IDRIS2_COMMIT" 2>/dev/null)" || true

        if [ -n "${BREW_COLLECTION}" ] && [ -n "${BREW_IDRIS2_COMMIT}" ]; then
          PACK_STATE="${PACK_STATE_DIR:-${XDG_STATE_HOME:-$HOME/.local/state}/pack}"
          STAMP="${PACK_STATE}/.brew-stamp"

          if [ "$(cat "${STAMP}" 2>/dev/null)" != "${BREW_COLLECTION}" ]; then
            mkdir -p "${PACK_STATE}"

            # 1. Point pack at the bundled collection
            printf 'collection = "%s"\n' "${BREW_COLLECTION}" > "${PACK_STATE}/pack.toml"

            # 2. Symlink bundled Idris2 toolchain to avoid 15-20 min rebuild
            IDRIS2_DIR="${PACK_STATE}/install/${BREW_IDRIS2_COMMIT}/idris2"
            if [ ! -e "${IDRIS2_DIR}" ]; then
              mkdir -p "${PACK_STATE}/install/${BREW_IDRIS2_COMMIT}"
              ln -sfn "${LIBEXEC}/idris2-toolchain" "${IDRIS2_DIR}"
            fi

            # 3. Clear stale collection DB so pack re-downloads on next run
            if [ -d "${PACK_STATE}/db" ]; then
              rm -rf "${PACK_STATE}/db"
            fi

            # 4. Write stamp to skip setup on subsequent runs
            printf '%s' "${BREW_COLLECTION}" > "${STAMP}"

            echo "pack: aligned state with collection ${BREW_COLLECTION}" >&2
          fi
        fi
